...
- **遭遇战**：1~7 个 AI 槽位（RA2 式槽位配置：颜色/国家/移除/添加），地图预览（更换一张/尺寸/类型），初始资金、游戏速度可调；10 个国家（各有特色单位）分属盟/苏/中三阵营
- **战役模式**：24 个任务（中国 8 / 盟军 8 / 苏军 8），固定种子地图 + 波次增援脚本 + 触发器系统（条件：计时/全灭/进区/资金；动作：刷兵/播报/给钱/开图/胜负），部分关卡为 `maps/*.txt` 手工地图
...
- **三阵营十国**：盟军 / 苏军 / 中国，差异化单位与建筑树；国家特色单位（坦克杀手/自爆卡车/恐怖分子/狙击手等）
...
- **遭遇战 AI**：建造序列（雷达后插海军船厂，失败自动放弃）、经济优先级（攒高科/超武）、编队进攻、超武释放
...
.\build\Release\ra2.exe --smoke-campaign 22 5000   # 无头跑战役第 22 关 5000 帧
